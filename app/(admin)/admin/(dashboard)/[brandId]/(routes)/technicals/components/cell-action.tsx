...
import { TechnicalsColumn } from "./columns";
import { useState } from "react";
import axios from "axios";
import { AlertModal } from "@/app/(admin)/admin/components/modals/alert-modal";
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuLabel, DropdownMenuTrigger } from "@/app/(admin)/admin/components/ui/dropdown-menu";
...
export const CellAction: React.FC<CellActionProps> = ({
  ...
  const onConfirm = async () => {
    try {
      ...
      const API_EDITED2 = API_EDITED.replace('{technicalId}', data.id)
      const response = await axios.delete(API_EDITED2);
      
      if(response.data === 'duplicate'){
        toast.error("Duplicate Technicals")
      }
  ...
  const onCopy = (id: string) => {
    ...
    toast.success('Technical ID copied to clipboard.');
  }
    ...
    <>
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          ...
          <DropdownMenuItem
            onClick={() => router.push(`${process.env.NEXT_PUBLIC_ADMIN_FOLDER_URL}/${params.brandId}/technicals/${data.id}`)}
          >
