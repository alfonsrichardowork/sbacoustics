...
const ComparisonPageSBAcoustics = () => {
            ...
            try {
                ...
                await Promise.all(
                    slugArray.map(async (value) => {
                        let temp: SingleProducts = await getProductComparison("", value);
    ...
    return (
                ...
                {finalFetchedProducts.map((product) => (
                  ...
                  >
                    <div className="flex flex-col items-center justify-center w-full">
                      ...
                      <Image
                        src={
                          product.coverImg.startsWith("/uploads/")
                            ? `${process.env.NEXT_PUBLIC_ROOT_URL}${product.coverImg}`
                            : product.coverImg
                        }
                        alt={`${product.name} - Comparison`}
                        width={100}
                        height={100}
                        className="
                          md:min-w-[200px]
                ...
                <React.Fragment key={`${parentKey}-${subKey}`}>
                  ...
                  <TableRow className="border-y-2 border-foreground">
                    <TableCell
                      ...
                      className="font-bold md:text-lg text-sm bg-zinc-100"
                    >
                      {parentKey}
                      {subKey !== "" && (
                        <> - <span className="font-semibold md:text-base text-xs text-gray-500">{subKey}</span></>
                      )}
                    </TableCell>
                  </TableRow>
      ...
      `}</style>
        ...
        <div className="pt-6 flex justify-center">
          <Button className="flex items-center md:text-base text-xs" onClick={() => router.push(`/drivers`)}>Add More Products</Button>
      </div>
        </>
        }
        </div>
...
export default ComparisonPageSBAcoustics;
