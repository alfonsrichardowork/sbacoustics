...
const SearchBox: FC<PropType> = (props) => {
  ...
  const miniSearchIndex = useMemo(() => {
    if (finalProductSearchbox.length === 0) return null;

    const ms = new MiniSearch({
      fields: ['label', 'slug', 'namenospaceStr', 'info', 'sizeStr', 'catStr', 'subcatStr', 'subsubcatStr', 'productInKitsStr'], 
      storeFields: ['slug'], 
      // Tokenizes input values dynamically down to space-isolated fragments
      tokenize: (string) => string.toLowerCase().split(/[\s-]+/g),
    ...
    const indexableData = finalProductSearchbox.map(item => ({
      ...item,
      ...
      namenospaceStr: joinArray(item.namenospace),
    }));
  ...
  function searchData(val: string) {
    ...
    if (updatedVal && miniSearchIndex) {
      // Execute micro full-text query
      const searchResults = miniSearchIndex.search(updatedVal, {
        prefix: true, // Matches variations like "sub" finding "subwoofer"
        fuzzy: 0.2,   // Safe, tight typo tolerance configuration
      });

      // Map matching IDs back to full dataset structures while preserving engine score rank
      const datasetMap = new Map(finalProductSearchbox.map(item => [item.slug, item]));
      results = searchResults
        .map(res => datasetMap.get(res.slug))
        .filter((item): item is ExtendedSearchbox => !!item);
    } else {
      results = finalProductSearchbox;
    }
