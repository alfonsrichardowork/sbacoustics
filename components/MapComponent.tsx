...
export const DistributorMap: React.FC<DistributorProps> = ({
  ...
  return (
    ...
    <div className="py-16 xl:px-16 lg:px-12 px-8 ">
      <h1 className="md:text-3xl text-2xl font-bold mb-16 text-center">
        Our Distributors
      </h1>

      <div className="grid md:grid-cols-4 gap-4 max-w-6xl mx-auto text-left">
        {allD.map((region, index) => {
          ...
          return (
            <section className="region-column" key={index}>
              <h2
                className={`md:font-bold font-semibold md:text-2xl text-xl md:pb-4 md:pt-0 ${
                  index === 0 ? "pt-0" : "pt-8"
                } pb-0`}
              >
                {titleDistributors[index]}
              </h2>

              {region.map((distributor, idx) => {
                return (
                  <React.Fragment key={idx}>
                    <h3 className="md:text-sm text-xs font-semibold md:pt-10 pt-4">
                      {distributor.country}
                    </h3>

                    <div
                      className="country-block hover:cursor-pointer hover:text-primary hover:font-bold"
                      onClick={() => {
                        setActiveMap(distributor);
                        setIsLoadingLoader(true);
                        handleScrollToTop();
                        mapRef.current?.flyTo(
                          [Number(distributor.lat), Number(distributor.lng)],
                          3,
                          { duration: 1.5 }
                        );
                      }}
                    >
                      <h4 className={`md:text-sm text-xs underline ${activeMap === distributor? 'text-primary font-bold' : ''}`}>
                        {distributor.name}
                      </h4>
                    </div>
                  </React.Fragment>
